...
impl FloodFillDegrees {
    ...
    pub const VARIANTS: [FloodFillDegrees; 2] =
        [
...
pub fn floodfill(
    ...
    while let Some(point) = stack.pop() {
        ...
        if degrees == FloodFillDegrees::Eight {
            if x < w - 1. && y < h - 1. { neighbors.push((x + 1., y + 1.)) };
            if x > 0. && y > 0. { neighbors.push((x - 1., y - 1.)) };
            if y < h - 1. && x > 0. { neighbors.push((x - 1., y + 1.)) };
            if y > 0.  && x < w - 1. { neighbors.push((x + 1., y - 1.)) };
        }
