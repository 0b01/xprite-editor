...
impl SymmetryMode {
    ...
    pub fn get_graph(&self, w: f64, h: f64) -> Vec<Rect> {
        match *self {
            ...
            SymmetryMode::Quad(m, n) => vec![Rect(vec2f!(m, 0), vec2f!(m, w)), Rect(vec2f!(0, n), vec2f!(h, n))],
            SymmetryMode::Diagonal(m) => {
                let p1 = if h <= w { vec2f!(m + w, w) } else { vec2f!(h, h - m) };
                vec![Rect(vec2f!(m, 0), p1)]
            }
            SymmetryMode::AntiDiagonal(m) => {
                let p1 = if w <= h { vec2f!(m - w, w) } else { vec2f!(0, m) };
                vec![Rect(vec2f!(m, 0), p1)]
            }
            SymmetryMode::Rotational(pivot, deg, max_) => {
                let radian = deg / 180. * PI;
...
mod tests {
    ...
    fn test_process_symmetry_vertial() {
        use super::*;
        let pixs = pixels!(pixel!(0, 0, Color::red()), pixel!(1, 0, Color::red()), pixel!(2, 0, Color::red()));
        let mut ret = Pixels::new();
        let symm = SymmetryMode::Vertical(1.);
        symm.process(&pixs, &mut ret);
        assert_eq!(ret, pixels!(pixel!(0, 1, Color::red()), pixel!(1, 1, Color::red()), pixel!(2, 1, Color::red())));
    }

    #[test]
    fn test_process_symmetry_horizontal() {
        use super::*;
        let pixs = pixels!(pixel_xy!(0, 0, Color::red()), pixel_xy!(1, 0, Color::red()), pixel_xy!(2, 0, Color::red()));
        let mut ret = Pixels::new();
        let symm = SymmetryMode::Horizontal(1.);
        symm.process(&pixs, &mut ret);
        assert_eq!(
            ret,
            pixels!(pixel_xy!(0, 1, Color::red()), pixel_xy!(1, 1, Color::red()), pixel_xy!(2, 1, Color::red()))
        );
    }
    ...
    fn test_process_symmetry_quad() {
        use super::*;
        let pixs = pixels!(pixel_xy!(0, 0, Color::red()));
        let mut ret = Pixels::new();
        let symm = SymmetryMode::Quad(1., 1.);
        symm.process(&pixs, &mut ret);
        assert_eq!(
            ret,
            pixels!(pixel_xy!(0, 1, Color::red()), pixel_xy!(1, 1, Color::red()), pixel_xy!(1, 0, Color::red()))
        );
    }
    ...
    fn test_antidiagonal_symmetry() {
        ...
        symm.process(&pixs, &mut ret);
        assert_eq!(ret, pixels!(pixel!(1, 1, Color::red())));
    }
    ...
    fn test_antidiagonal_symmetry2() {
        ...
        symm.process(&pixs, &mut ret);
        assert_eq!(ret, pixels!(pixel!(2, 2, Color::red())));
    }
    ...
    fn test_antidiagonal_symmetry3() {
        ...
        let symm = SymmetryMode::AntiDiagonal(3.);
        symm.process(&pixs, &mut ret);
        assert_eq!(ret, pixels!(pixel!(2, 1, Color::red()), pixel!(2, 2, Color::red())));
    }

    #[test]
    fn test_diagonal_symmetry() {
        use super::*;
        let pixs = pixels!(pixel!(0, 1, Color::red()));
        let mut ret = Pixels::new();
        let symm = SymmetryMode::Diagonal(0.);
        symm.process(&pixs, &mut ret);
        assert_eq!(ret, pixels!(pixel!(1, 0, Color::red())));
    }
