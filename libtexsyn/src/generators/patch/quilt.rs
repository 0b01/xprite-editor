...
use rand::{thread_rng, Closed01, Rand, Rng};
use rayon::prelude::*;

use std::collections::HashMap;
use std::sync::Mutex;

use common::{blit_rect, OrderedFloat, Patch, Rect};
...
impl Quilter {
    ...
    pub fn quilt_image(&mut self) -> Result<RgbImage, String> {
        ...
        let x_patches = if (self.params.size.0 % step) == 0 {
            self.params.size.0 / step
        } else {
            self.params.size.0 / step + 1
        };
        let y_patches = if (self.params.size.1 % step) == 0 {
            self.params.size.1 / step
        } else {
            self.params.size.1 / step + 1
        };
        let (buffer_width, buffer_height) = (
            self.params.size.0 + self.params.patch_size,
            self.params.size.1 + self.params.patch_size,
        );
        ...
        blit_rect(
            ...
            &Rect {
                coords: if let Some(seed_coordinates) = self.params.seed_coords {
                    ...
                    (
                        patch_x_dist.ind_sample(&mut rng),
                        patch_y_dist.ind_sample(&mut rng),
                    )
                },
        ...
        Ok(quilt
            .sub_image(0, 0, self.params.size.0, self.params.size.1)
            .to_image())
    }
    ...
    fn patch_error(&self, area: OverlapArea, patch: &Patch, buf_coords: (u32, u32)) -> f64 {
        ...
        match area {
            ...
            OverlapArea::Left => patch_rect_error(
                self.params.distance_func,
                &self.source,
                buffer,
                patch.coords,
    ...
    fn select_candidate(&self, area: OverlapArea, buf_coords: (u32, u32)) -> Patch {
        ...
        let mut candidates: Vec<Patch> = scores
            .into_iter()
            .filter_map(|(p, err)| {
                if err > *best * (1. + TOLERANCE) {
                    None
                } else {
                    Some(p.clone())
                }
    ...
    fn minimum_cost_vertical_path(
        &self,
        err_surf: &ErrorSurface,
    ) -> Result<Vec<(u32, u32)>, String> {
        let mut v = vec![];
        let cost_map = self.vertical_cost_map(err_surf);

        // Find path starting point
        let row = (0..self.params.overlap)
            .into_iter()
            .map(|x| cost_map[&(x, self.params.patch_size - 1)])
            .collect::<Vec<_>>();
        let (mut x, mut y) = (
            row.into_iter()
                .enumerate()
        ...
        while y != 0 {
            ...
            if x == 0 {
                let right = cost_map
                    .get(&(x + 1, y - 1))
                    .ok_or_else(|| "cost map item not found".to_owned())?;
                if right < &top {
                    x += 1;
                }
    ...
    fn minimum_cost_horizontal_path(
        &self,
        err_surf: &ErrorSurface,
    ) -> Result<Vec<(u32, u32)>, String> {
        let mut v = vec![];
        ...
        // Find path starting point
        let column = (0..self.params.overlap)
            .into_iter()
            .map(|y| cost_map[&(self.params.patch_size - 1, y)])
            .collect::<Vec<_>>();
        let (mut x, mut y) = (
            self.params.patch_size - 1,
            column
        ...
        let mut do_pixel = |x, y| {
            ...
            if y >= hpos.1 && x >= vpos.0 {
                buffer.put_pixel(
                    ...
                    *self
                        .source
                        .get_pixel(patch.coords.0 + x, patch.coords.1 + y),
                );
            }
        };
        for x in 0..overlap {
    ...
    fn cut_and_blit_patch(
        ...
        match area {
            ...
            OverlapArea::TopLeft => {
                let (vpath, vpath_corner): (Vec<_>, Vec<_>) = self
                    .minimum_cost_vertical_path(err_surf)?
                    .into_iter()
                    .partition(|&(_, y)| y >= overlap);
                let (hpath, hpath_corner): (Vec<_>, Vec<_>) = self
                    .minimum_cost_horizontal_path(err_surf)?
                    .into_iter()
