...
impl Renderer for StdwebRenderer {
    ...
    fn rect(&mut self, p0: [f64; 2], p1: [f64; 2], color: [f32; 4], filled: bool) {
        ...
        if filled {
            if self.last.is_none() {
                let col = format!("rgba({},{},{},{})", color[0] * 255., color[1] * 255., color[2] * 255., color[3],);
    ...
    fn bezier(&mut self, p0: [f64; 2], cp1: [f64; 2], cp2: [f64; 2], p1: [f64; 2], color: [f32; 4], thickness: f64) {
        ...
        self.ctx.move_to(p0[0] as f64, p0[1] as f64);
        self.ctx
            .bezier_curve_to(cp1[0] as f64, cp1[1] as f64, cp2[0] as f64, cp2[1] as f64, p1[0] as f64, p1[1] as f64);
        self.ctx.stroke();
    }
}
...
impl StdwebRenderer {
    pub fn new(attr_id: &str) -> Self {
        let canvas: CanvasElement = document().query_selector(attr_id).unwrap().unwrap().try_into().unwrap();
